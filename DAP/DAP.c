...
static uint32_t DAP_SWJ_Clock(const uint8_t *request, uint8_t *response) {
  ...
  if (clock == 5000) {// select swd clock 5k AS 24M max clock freq
    DAP_Data.fast_clock  = 1U;
    DAP_Data.clock_delay = 1U;
  } else {
    DAP_Data.fast_clock  = 0U;
    DAP_Data.clock_delay = CPU_CLOCK/10 / clock ;
  }
...
void PORT_OFF(void)
{
    GPIO_InitTypeDef GPIO_SWD = { 0 };
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);
    GPIO_SWD.GPIO_Pin = SWCLK_PIN | SWDIO_PIN;
    GPIO_SWD.GPIO_Mode = GPIO_Mode_IPU;
    GPIO_SWD.GPIO_Speed = GPIO_Speed_10MHz;
    GPIO_Init(SWD_PORT, &GPIO_SWD);
}
...
void DAP_SETUP(void)
{
    ...
    GPIO_InitTypeDef GPIO_SWD = { 0 };
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA, ENABLE);
    GPIO_SWD.GPIO_Pin = SWCLK_PIN | SWDIO_PIN | SWRST_PIN;
    GPIO_SWD.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_SWD.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(SWD_PORT, &GPIO_SWD);
    ...
    GPIO_LED.GPIO_Pin = LED_CONNECTED_PIN | LED_RUNNING_PIN;
    GPIO_LED.GPIO_Mode = GPIO_Mode_Out_PP;
    GPIO_LED.GPIO_Speed = GPIO_Speed_10MHz;
    GPIO_Init(LED_PORT, &GPIO_LED);

    LED_CONNECTED_OUT(0);//trun off led
    LED_RUNNING_OUT(0);//trun off led
}
...
{
    if (bit&1) {
        LED_PORT->BCR = LED_CONNECTED_PIN;
    } else {
        LED_PORT->BSHR = LED_CONNECTED_PIN;
    }
}
...
{
    if (bit&1) {
        LED_PORT->BCR = LED_RUNNING_PIN;
    } else {
        LED_PORT->BSHR = LED_RUNNING_PIN;
    }
}
